...
export async function getMemory(sessionId) {
  return request(`/api/session/${sessionId}/memory`, { method: "GET" }); // { memory: {A, B}, arena }
}

export async function getAuthority(sessionId) {
  return request(`/api/session/${sessionId}/authority`, { method: "GET" }); // { mode, refereeEnabled, lastRealityEvent }
}

export async function setAuthority(sessionId, { mode, refereeEnabled } = {}) {
  return request(`/api/session/${sessionId}/authority`, {
    method: "PUT",
    body: JSON.stringify({ mode, refereeEnabled }),
  });
}
