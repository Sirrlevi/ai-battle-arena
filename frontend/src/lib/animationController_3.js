...
// M1 PATCHED - backward compatible + physics aware
export function createAnimState(x, y, groundY, physicsProfile = null) {
  const baseMotion = createMotionState(x, y, groundY, physicsProfile);
  return {
    motion: baseMotion,
    attackPhase: null,
    pendingImpact: null,
...
export function setAnimPhysicsProfile(anim, physicsProfile) {
  ...
  if (anim.motion) anim.motion.physicsProfile = physicsProfile;
}
...
export function applyHitReaction(anim, fromX, damage = 0, knockbackMultiplier = 1) {
  // M1 backward compatible: detect if called with (anim, damage, attackerAnim, impact)
  if (typeof fromX === 'number' && typeof damage === 'number' && typeof knockbackMultiplier === 'number') {
    // old signature: (anim, fromX, damage, knockbackMultiplier)
    anim.hitTimer = HIT_REACT_DURATION;
    anim.flashTimer = FLASH_DURATION;
    anim.lastHitDamage = damage;
    const dir = anim.motion.x >= fromX ? 1 : -1;
    anim.hitDir = dir;
    // M1: if physicsProfile has heavy mass, reduce knockback a bit
    const massFactor = anim.physicsProfile ? Math.max(0.4, 150/(anim.physicsProfile.mass||75)) : 1;
    anim.motion.vx = dir * KNOCKBACK_SPEED * knockbackMultiplier * massFactor;
    return { knockback: KNOCKBACK_SPEED * knockbackMultiplier, dir, fallDirection: 'back' };
  } else {
    // new signature: (anim, damage, attackerAnim, impact) where fromX is actually damage
    const actualDamage = fromX;
    const attackerAnim = damage;
    const impact = knockbackMultiplier;
    anim.hitTimer = HIT_REACT_DURATION;
    anim.flashTimer = FLASH_DURATION;
    anim.lastHitDamage = actualDamage;
    // calculate real direction
    let dir = 1;
    let fallDirection = 'back';
    try {
      const { calculateImpact } = require('./impactSystem.js'); // will be imported dynamically in App, but we have fallback
    } catch(e){}
    if (attackerAnim && attackerAnim.motion) {
      dir = anim.motion.x >= attackerAnim.motion.x ? 1 : -1;
      anim.hitDir = dir;
      // check if it's down (Bug4)
      const isDown = anim.knockdownPhase === 'down' || anim.isSliding;
      if (isDown) {
        // slide, no pop-up
        const slideDist = impact?.slideDistance || 60;
        anim.isSliding = true;
        if (anim.motion) {
          anim.motion.isSliding = true;
          anim.motion.vx = dir * (impact?.knockbackForce*0.18 || 80);
          anim.motion.y = anim.motion.groundY;
          anim.motion.grounded = true;
        }
        anim.fallDirection = 'back';
        return { knockback: slideDist, dir, fallDirection: 'back', isSliding: true };
      }
      // heavy mass factor
      const massFactor = anim.physicsProfile ? Math.max(0.4, 150/(anim.physicsProfile.mass||75)) : 1;
      anim.motion.vx = dir * (impact?.launchVelocity?.x || KNOCKBACK_SPEED) * massFactor;
      if (impact?.launchVelocity?.y && !isDown) {
        anim.motion.vy = impact.launchVelocity.y * 0.5;
        anim.motion.grounded = false;
      }
      fallDirection = impact?.fallDirection || 'back';
      anim.fallDirection = fallDirection;
    ...
    const dir2 = 1;
    anim.hitDir = dir2;
    anim.motion.vx = dir2 * KNOCKBACK_SPEED;
    anim.fallDirection = 'back';
    return { knockback: KNOCKBACK_SPEED, dir: dir2, fallDirection: 'back' };
  }
}
...
export function triggerKnockdown(anim, fromX, damage, knockbackMultiplier) {
  // detect signature
  if (typeof fromX === 'number' && typeof damage === 'number') {
    // old: (anim, fromX, damage, knockbackMultiplier)
    anim.hitTimer = HIT_REACT_DURATION;
    anim.flashTimer = FLASH_DURATION;
    anim.lastHitDamage = damage;
    const dir = anim.motion.x >= fromX ? 1 : -1;
    anim.hitDir = dir;
    const massFactor = anim.physicsProfile ? Math.max(0.4, 150/(anim.physicsProfile.mass||75)) : 1;
    anim.motion.vx = dir * KNOCKBACK_SPEED * (knockbackMultiplier||1) * massFactor;
    anim.knockdownPhase = "falling";
    anim.knockdownTimer = KNOCKDOWN_FALL_DURATION;
    anim.fallDirection = 'back'; // M1 fix: always back for front hit
    return;
  } else {
    // new: (anim, damage, attackerAnim, impact)
    const actualDamage = fromX;
    const attackerAnim = damage;
    const impact = knockbackMultiplier;
    anim.hitTimer = HIT_REACT_DURATION;
    ...
    anim.hitDir = dir;
    anim.motion.vx = dir * (impact?.launchVelocity?.x || KNOCKBACK_SPEED*1.5);
    if (impact?.launchVelocity?.y) {
      anim.motion.vy = impact.launchVelocity.y;
      anim.motion.grounded = false;
    }
    anim.knockdownPhase = "falling";
    anim.knockdownTimer = KNOCKDOWN_FALL_DURATION;
