...
// ---------- DYNAMIC PHYSICS PROFILE GENERATOR - M1 CORE REWRITE ----------
// Replaces fixed mass with a Dynamic Physics Profile derived automatically
// from the Combat Profile. No hardcoded roster.
...
function pickWeightClass(combatProfile, character) {
  ...
  // special overrides
  if (combatProfile?.realityManipulation || combatProfile?.timeManipulation) {
    if (tier.includes('Conceptual') || tier.includes('Narrative') || tier.includes('Author') || tier.includes('Multiversal')) {
      if (persona.includes('ghost')||persona.includes('spirit')||species.includes('ethereal')) return 'Ethereal';
      if (persona.includes('energy')||persona.includes('light')||persona.includes('cosmic')) return 'Cosmic';
    }
  }
  ...
  // strength/durability bias within tier options
...
function deriveBodyType(combatProfile, character) {
  ...
  if (s.includes('robot')||s.includes('mech')||s.includes('cyborg')||s.includes('android')) return 'mechanical';
  if (s.includes('dragon')||s.includes('beast')||s.includes('monster')) return 'beast';
  if (s.includes('energy')||s.includes('light being')) return 'energy';
  if (s.includes('ghost')||s.includes('spirit')||s.includes('phantom')) return 'ethereal';
  if (s.includes('giant')||s.includes('titan')||s.includes('colossal')) return 'giant';
  if (s.includes('quadruped')||s.includes('wolf')||s.includes('tiger')) return 'quadruped';
...
export function generatePhysicsProfile(combatProfile, character = {}) {
  ...
  // density variation from durability + tier
  ...
  // Height: base human 80px rig scale, but varies
  ...
  // agility reduces apparent bulk
  heightFactor *= (0.9 + (mobility/10)*0.2);
  
  const height = 80 * heightFactor;
  const reach = (weightClassName === 'Titan' || weightClassName === 'Colossal') ? 78 : 
                weightClassName === 'Tiny' ? 32 : 
                52 + (mobility*1.5) + (strength*1.2);
  ...
  const groundPressure = mass / 100; // simplified
  ...
  const flightPhysics = {
    canFly: !!combatProfile?.flight,
    canHover: !!combatProfile?.flight || (mobility>=8 && speed>=7),
    lift: combatProfile?.flight ? 1.0 + (mobility/10)*0.5 : 0,
    drag: weightDef.isEthereal ? 0.02 : weightDef.isEnergy ? 0.08 : 0.12,
    maxAltitude: combatProfile?.flight ? 300 : 80,
    airControl: agility * 0.8 + 0.2,
  };
  const collisionBehaviour = {
    separationForce: Math.max(4, Math.min(25, mass*0.02 + 5)),
    wallBounceRestitution: weightDef.isEthereal ? 0.05 : weightDef.isEnergy ? 0.15 : Math.max(0.1, Math.min(0.5, 0.5 - mass/10000)),
    groundFriction: groundGrip,
    canOverlap: weightDef.isEthereal,
  };
  const hitboxScale = heightFactor;
  const centerOfMass = { x: 0, y: weightDef.isEthereal ? -70 : weightDef.isEnergy ? -80 : -55 }; // hips is -55 per RIG.HIP_Y - this is THE FIX for Bug1
  const proceduralAnimationPreset = {
    stanceWidth: 0.85 + (balance*0.3) + (weightClassName==='Heavy'?0.15:0),
    lean: (strength>=7?-2:2),
    strideScale: 0.8 + agility*0.4,
    crouchOnHeavy: weightClassName==='Heavy'||weightClassName==='Titan',
  };
  const ragdollPreset = {
    root: 'hips', // FIX: was head, now hips per Bug1
    pivotStiffness: Math.min(0.95, 0.4 + impactResistance*0.5),
    fallStyle: weightClassName==='Ethereal' ? 'float' : weightClassName==='Energy' ? 'disperse' : 'weighted',
    hipsAnchor: true,
  };
  ...
  return {
    ...
    weightDef,
    bodyType: deriveBodyType(combatProfile, character),
    bodySize: heightFactor <0.7?'small':heightFactor>1.8?'huge':heightFactor>1.3?'large':'medium',
    mass,
    density,
    height,
    reach,
    balance,
    mobility: mobilityNorm,
    agility,
    groundGrip,
    groundPressure,
    impactResistance,
    ...
    ragdollPreset,
    // for debugging
    derivedFrom: { tier: combatProfile?.combatTier, strength, durability, speed, mobility },
...
// Utility to compute effective weight class scaling for damage/knockback
